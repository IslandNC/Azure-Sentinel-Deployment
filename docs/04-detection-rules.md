...
|---|---|---|---|---|
| Entra ID – User Sign-in from Different Location | High | T1078 — Valid Accounts | `queries/identity/impossible-travel-detection.kql` | Enabled |
| RDP Brute Force Attack | Medium | T1110.001 — Password Guessing | `queries/identity/brute-force-signin-1h.kql` | Enabled |
| SSH Brute Force on Linux | Low | T1110.001 — Password Guessing | `queries/identity/brute-force-signin-5m.kql` | Enabled |
    ...
    2. Check MFA status on both sign-ins
      3. Review other activity from both source IPs
        4. Confirm whether the sign-in device is known/managed
          ...
          - **Severity:** Medium
          - **Rule Type:** Custom detection rule
          - **Scheduling:** Runs every 1 hour, looks back 1 hour
          - **Incident Correlation:** Enabled
          ...
          - **Alert Trigger:** `FailedAttempts >= 5` within 1 hour from same IP
                        ...
                        2. Review `/var/log/auth.log` on linux-vm for targeted usernames
                          3. Confirm no successful SSH logon from the attacking IP
                            4. Correlate with any lateral movement indicators on other VMs
                              ...
                              - The 5-minute scheduling on the SSH rule is appropriate for a fast-running brute force; the 1-hour schedule on the RDP rule suits slower, distributed attacks
                              ...
                              Additional rules to consider for a more complete detection coverage:
